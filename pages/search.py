...
def create_graph_data():
    ...
    node_data = {
        ...
        "query_sentences": list(
            set(itertools.chain.from_iterable(df.head(n=top)["q_sent_text"]))
        ),
    ...
    for k in node_data:
        ...
        for v in node_data[k]:
            # logger.debug(f'{k} {v}')
            element_data.append({"data": {"id": v, "label": v[:30]}, "classes": k},)

    # create links
    for i, row in df.head(n=top).iterrows():
        element_data.append({"data": {"source": row["Name"], "target": row["Org"][0]}},)
