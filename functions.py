...
def api_search_person(text: str):
    ...
    if not df.empty:
        ...
        # logger.debug(f'\n{df.head()}')
        df.rename(
            columns={"name": "Name", "person_id": "ID", "score": "Score", "org": "Org"},
            inplace=True,
        )
...
def api_search_output(text: str):
    ...
    if not df.empty:
        ...
        df["title"] = df["title"]
        df.rename(
            columns={"title": "Title", "year": "Year", "score": "Score",}, inplace=True
        )
...
def collab_tsne(df):
    ...
    aaa_df = pd.json_normalize(aaa.json()["res"])
    logger.info(aaa_df)
    # run tSNE
    ...
    df["ScoreNorm"] = round(
        (df["Score"] - df["Score"].min()) / (df["Score"].max() - df["Score"].min()), 4
    )
...
def plotly_scatter_plot(df, top=1):
    ...
    logger.info(f"\n{df.head()}")
    fig = px.scatter(
        df, x="x", y="y", color="org-name", symbol="org-name", hover_data=["name"]
    )
...
def run_tsne(query: str = "data science"):
    ...
    pp_df = pp_df[
        (pp_df["person_id1"].isin(person_check))
        & (pp_df["person_id2"].isin(person_check))
    ]
    ...
    summary_df["org-name"] = (
        summary_df["org-name"].astype(str) + " " + summary_df["org_count"].astype(str)
    )
